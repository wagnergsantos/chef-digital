-- Migration: Adicionar suporte a grupos de ingredientes
-- Data: 2026-08-25

ALTER TABLE public.ingredientes ADD COLUMN IF NOT EXISTS group_name TEXT DEFAULT NULL;

-- Atualizar function salvar_receita
...
BEGIN
    ...
    IF p_ingredientes IS NOT NULL AND jsonb_array_length(p_ingredientes) > 0 THEN
        ...
        LOOP
            INSERT INTO ingredientes (receita_id, name, qty, unit, group_name, ordem)
            VALUES (
                v_recipe_id,
                v_item->>'name',
                (v_item->>'qty')::NUMERIC,
                v_item->>'unit',
                NULLIF(TRIM(v_item->>'group_name'), ''),
